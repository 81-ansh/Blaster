...
{
	return (EquippedWeapon != nullptr && SecondaryWeapon != nullptr && !bAiming && (CombatState == ECombatState::ECS_Unoccupied || CombatState == ECombatState::ECS_Reloading));
}
...
void UCombatComponent::AttachActorToRightHand(AActor* ActorToAttach)
{
	...
	if (HandSocket)
	{
		HandSocket->AttachActor(ActorToAttach, Character->GetMesh());									// Attach weapon to the hand socket
	}
}
...
void UCombatComponent::AttachActorToLeftHand(AActor* ActorToAttach)
{
	...
	const USkeletalMeshSocket* HandSocket = Character->GetMesh()->GetSocketByName(SocketName);					// Get the hand socket from the character mesh
	if (HandSocket)
	{
		HandSocket->AttachActor(ActorToAttach, Character->GetMesh());									// Attach weapon to the hand socket
	}
...
void UCombatComponent::UpdateShotgunAmmoValues()
{
	...
	bCanFire = true;		// So that shotgun can be fired even after 1 ammo is loaded
...
int32 UCombatComponent::AmountToReload()
{
	...
	int32 RoomInMag = EquippedWeapon->GetMagCapacity() - EquippedWeapon->GetAmmo();		// How much space is available for ammo
	if (CarriedAmmoMap.Contains(EquippedWeapon->GetWeaponType()))					// Check if there is ammo for the certain weapon
	{
		int32 AmountCarried = CarriedAmmoMap[EquippedWeapon->GetWeaponType()];			// Get the ammo in the Weapon
...
void UCombatComponent::ThrowGrenade()
{
	...
	if (Character && Character->HasAuthority())
	{
		Grenade = FMath::Clamp(Grenade -1, 0, MaxGrenades);
		UpdateHUDGrenade();
	}
}
